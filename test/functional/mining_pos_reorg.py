...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected WCH and zWCH supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zWCHsupply']:
                assert_equal(node['zWCHsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check WCH and zWCH supply at the beginning
        # ------------------------------------------
        # zWCH supply: 2 coins for each denomination
        ...
        # WCH supply: block rewards minus burned fees for minting
        ...
        # Verify that WCH and zWCH supplies were properly updated after the spends and reorgs
        self.log.info("Check WCH and zWCH supply...")
