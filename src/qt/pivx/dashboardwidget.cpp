    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of WCH earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 WCH");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("WCH")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair WCH, zWCH
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(57, 209, 150));
	set1->setColor(QColor(51, 161, 119));
