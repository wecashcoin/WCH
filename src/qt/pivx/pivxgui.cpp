...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("WeCash client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("WeCash"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("WeCash"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
