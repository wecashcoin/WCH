...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 1200000)	return 2   * COIN;
    if (nHeight > 800000)	return 3   * COIN;
    if (nHeight > 400000)	return 4   * COIN;
    if (nHeight > 300000)	return 5   * COIN;
    if (nHeight > 200000)	return 6   * COIN;
    if (nHeight > 100000)	return 5   * COIN;
    if (nHeight > 43200)	return 4   * COIN;
    if (nHeight > 28800)	return 3   * COIN;
    if (nHeight > 14400)	return 2   * COIN;
...
{
    return GetBlockValue(nHeight) * 50 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 WCH tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 WCH tx -> 1 confirmation
