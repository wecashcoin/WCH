...
const char * const PIVX_CONF_FILENAME = "wch.conf";
const char * const PIVX_PID_FILENAME = "wch.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// WeCash only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\wch
// Windows >= Vista: C:\Users\Username\AppData\Roaming\wch
// Mac: ~/Library/Application Support/wch
// Unix: ~/.wch
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WeCash";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\wchParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\wchParams
    // Mac: ~/Library/Application Support/wchParams
    // Unix: ~/.wch-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WeCashParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "WeCashParams";
#else
    // Unix
    return pathRet / ".wch-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wch.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# WeCash config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wch.conf
