...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between WCH and zWCH
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // WCH: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
